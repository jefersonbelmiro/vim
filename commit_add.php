...
if ( empty($oParametros->sArquivo) ) {
  $aMensagemErro[] = 'Arquivo para commit não informado';
}
  
if ( empty($oParametros->sMensagem) ) {
  $aMensagemErro[] = 'Mensagem do commit não informado';
}

if ( empty($oParametros->iTag) || strlen($oParametros->iTag) < 4 ) {
  $aMensagemErro[] = 'Tag não informada, ou inválida';
}

if ( empty($oParametros->sTipoAbreviado) || empty($oParametros->sTipoCompleto) ) {
  $aMensagemErro[] = "Tipo de commit não informado | Tipos de commit: addeded(a, add), fixed(fix, f), enhanced(enh, e), style(s)";
}
